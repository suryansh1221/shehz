...
import { Eye, ExternalLink, Volume2, VolumeX, X } from "lucide-react";
...
export default function VaultGrid() {
  ...
  function handleSelect(item: VaultItem) {
    if (!item.youtubeId) setActiveItem(item);
  }
              ...
              >
                {item.youtubeId ? (
                  <div className="absolute inset-0">
                    <iframe
                      key={isUnmuted ? "unmuted" : "muted"}
                      src={`https://www.youtube.com/embed/${item.youtubeId}?autoplay=1&mute=${
                        isUnmuted ? 0 : 1
                      }&loop=1&playlist=${item.youtubeId}&controls=0&showinfo=0&modestbranding=1&iv_load_policy=3&playsinline=1&rel=0&disablekb=1`}
                      title={item.title}
                      allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                      referrerPolicy="strict-origin-when-cross-origin"
                      allowFullScreen
                      className="pointer-events-none absolute left-1/2 top-1/2 h-[177.78%] min-h-full w-[177.78%] min-w-full -translate-x-1/2 -translate-y-1/2 sm:h-[130%] sm:w-[130%]"
                    />
